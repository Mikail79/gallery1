...
@section('content')
    <div class="m-3">
                ...
                <div class="card shadow-sm" style="width: 18rem;">
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            ...
                            <div class="dropdown"> <button class="btn dropdown-toggle noarrow dropdown-btn" type="button"
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton-{{ $foto->id }}">
                                    <li>
                                        <form class="dropdown-item" action="{{ route('foto.remove', $foto->id) }}"
                                            method="POST"> @csrf @method('DELETE') <button class="btn" type="submit">
                                                <i class="bi bi-trash"></i> Delete </button> </form>
                                    </li>
                                    <li>
                                        <div class="btn"> <button type="button" class="btn" data-bs-toggle="modal"
                                                data-bs-target="#edit{{ $foto->id }}"> <i class="bi bi-pencil-fill"></i>
                                                Edit </button> </div>
                                    </li>
                                </ul>
                            </div>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h1 class="modal-title text-center fs-5" id="comment{{ $foto->id }}Label">
                                                {{ $foto->title }}</h1>
                                        ...
                                        <div class="modal-body">
                                            <img src="{{ asset($foto->file_location) }}" class="card-img-top"
                                                alt="...">
                                            <div class="my-3">
                                              <h4>Komentar</h4> <div class="card p-3 overflow-auto" style="max-height: 180px"> @if (count($foto->komentarFoto) == 0) <span class="text-center fw-bold">Tidak ada Komentar</span> @endif
        ...
        <div class="modal fade" id="edit{{ $foto->id }}" tabindex="-1" aria-labelledby="edit{{ $foto->id }}Label" aria-hidden="true">
          <form class="modal-dialog modal-xl modal-dialog-centered" action="{{ route('foto.update', $foto->id) }}" method="POST">
              @csrf
              @method('PUT')
              <div class="modal-content">
              <div class="modal-header">
              ...
              <div class="modal-body">
                      ...
                      <div class="mb-3">
                        ...
                        <input value="{{ $foto->title }}" type="text" name="title" class="form-control" placeholder="Nama Foto" aria-describedby="emailHelp">
                      </div>
                      <div class="mb-3 d-flex flex-column">
                        <label for="exampleInputPassword1" class="form-label">Deskripsi</label>
                        <textarea value class="form-control" name="description" id="" cols="20" rows="5">{{ $foto->description }}</textarea>
                      </div>
                    </form>
              </div>
              <div class="modal-footer">
